...
class MultiAgentAuditor:
    ...
    def run_preflight_tools(self) -> str:
        try:
            result = subprocess.run(
                ["make", "preflight"], capture_output=True, text=True, timeout=120
            )
            return f"STDOUT:\n{result.stdout}\nSTDERR:\n{result.stderr}"
    ...
    def generate_focused_context(self, modified_files: list[str]) -> str:
        ...
        try:
            ...
            return (
                f"Failed to generate focused context. Return code: "
                f"{e.returncode}\nSTDERR: {e.stderr}"
            )
        except Exception as e:
            return f"Failed to generate focused context: {str(e)}"

    async def _run_expert_agent(
        self, persona_prompt: str, diff: str, repo_context: str, test_logs: str
    ) -> str:
        prompt = f"{persona_prompt}\n\n### REPO ###\n{repo_context}\n\n"
        prompt += f"### LOGS ###\n{test_logs}\n\n### DIFF ###\n{diff}\n"

        chat = self.client.aio.chats.create(
            model=self.model_name, config=genai.types.GenerateContentConfig(temperature=0.1)
        )
    ...
    async def execute_audit(self):
                    ...
                    items=types.Schema(
                        ...
                        properties={
                            "title": types.Schema(
                                ...
                                description="Short, concise title of the fix",
                            ),
                            "description": types.Schema(
                                type=types.Type.STRING,
                                description="Detailed explanation of what needs to be fixed",
                            ),
                            "severity": types.Schema(
                                type=types.Type.STRING,
                                description="Severity of the issue: Critical, High, Medium, or Low",
                            ),
                            "ai_execution_instructions": types.Schema(
                                type=types.Type.STRING,
                                description="Instructions for AI on how to address this, "
                                "mandating the workflow: develop a plan, get it reviewed, "
                                "implement, ensure tests pass, and commit.",
                            ),
                        },
        ...
        strategic_prompt_full = (
        f"{STRATEGIC_LEAD_PROMPT}\n\n### PHYSICIST ###\n{physicist_report}\n\n"
        f"### ARCHITECT ###\n{architect_report}\n\n"
        f"### LOGS ###\n{test_logs}\n\n### DIFF ###\n{diff}\n"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["repo", "staged", "commits"],
        default="repo",
        help="Audit mode: 'repo' (repo), 'staged' (staged changes), or 'commits' (prev N "
        "commits).",
    )
    parser.add_argument(
        "--n-commits",
        type=int,
        default=1,
        help="Number of previous commits to audit (only used in 'commits' mode).",
    )
