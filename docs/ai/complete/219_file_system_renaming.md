...
3. Do a workspace-wide Find & Replace: Update all import paths from `src.benchmarks.aging_resilience` to `src.benchmarks.aging_resilience`.
4. Update all hardcoded output directories inside the scripts from `output/benchmarks/aging_resilience` to `output/benchmarks/aging_resilience`.
