...
2. Inside `execute_audit`, define two strict persona prompts:
   - **The Mathematical Physicist**: Instruct it to verify continuous-time Euler-Maruyama discretization (ZOH limits in `masr_mamba.py`), and ensure thermodynamic invariants (Q skew-symmetry in `solenoidal.py`, Gamma positive-definiteness via Cholesky in `dissipative.py`).
