ROLE: You are an elite Scientific Machine Learning Engineer specializing in JAX and differential geometry.

TASK: We are building `src/echo/metrics/thermal_interpretability.py`. Implement a robust, production-ready class called `HessianCurvatureTracker` designed to extract the topographical geometry of the biological attractor basin (the Waddington landscape) at runtime.
