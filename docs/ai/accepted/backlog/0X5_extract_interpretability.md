Files to tag in Composer: src/metrics/mamba_lrp.py, src/metrics/autopsy_engine.py
...
Please refactor `MambaLRPEpsilon` and `ThermodynamicAutopsyEngine` into a clean, general-purpose set of classes.
...
3. Remove all hardcoded biological feature names (e.g., "Sigma_PC", "Psi_TP53") from the autopsy engine. Allow the user to optionally pass a list of `feature_names` during initialization for the causal trace report.
