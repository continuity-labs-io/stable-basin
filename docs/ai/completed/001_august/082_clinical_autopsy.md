...
> Create a new file at `src/harness/clinical_diagnostic_runner.py`. You are an Expert PyTorch ML Engineer. Write a production-grade CLI harness with `argparse` and `logging`.
> Requirements:
...
> 6. Pass the engine and sequence to `ThermodynamicDiagnosticEngine` to generate a JSON diagnostic report for the `crash_frame`. Log a metric evaluating the causal coherence (e.g., sparsity/focus of the relevance tensor).
...
> 
> 8. Save the JSON report to `output/harness/clinical_diagnostic_report_[model_type].json`, the dashboard to `output/harness/[png_name]`, and a CSV summary to `output/harness/[csv_name]`.
