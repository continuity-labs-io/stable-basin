...
We need to build an Asynchronous Data Packer to natively process asynchronous, multi-rate biological sensors without massive zero-padding. Standard PyTorch forces us to pad high-frequency channels with fake zeros to create dense matrices, which wastes VRAM and compute multiplying by zero[cite: 1].

Write a pre-processing function that takes the asynchronous firehose and packs it into a flat, 1-dimensional Event Tensor[cite: 1]. Each event must be a tight tuple formatted exactly as: [Value, Sensor_ID, Hardware_Timestamp][cite: 1]. No zero-padding is allowed[cite: 1]. 

The deliverable should be a PyTorch Dataset or DataLoader class that outputs ragged event tensors and boolean sparsity masks[cite: 1]. Look at how HuggingFace uses document packing to train LLMs on texts of different lengths; we are doing the exact same thing, but for sensors instead of sentences[cite: 1].
...
Before we touch @triton.jit, we must establish the mathematical ground truth in pure PyTorch so torch.autograd.gradcheck has a target to verify against. Here is the exact codebase for the first two files.

1. src/data/async_event_packer.py
This wrapper strips out the padded zeros and outputs a flat, chronological event sequence.
