We are upgrading to `OrthogonalModalityEncoder` with Proportional Orthogonal
Routing. Please make the following updates:

**1. Update `src/models/encoders/orthonogonal_modality_encoder.py`** Replace the
existing fusion class with this implementation that safely handles edge cases in
...
2. Update src/models/simulators/sensor_fusion_predictor.py Update the
   SensorFusionPredictor initialization to correctly instantiate the new
   encoder. Change the **init** signature to accept modality_dims: list[int] =
   None.
...
3. write unit test verifying the edge case modality_dims=[1000, 2] with
   d_model=64 properly allocates exactly 63 channels to Modality 0, and 1
   channel to Modality 1 without throwing any errors.
