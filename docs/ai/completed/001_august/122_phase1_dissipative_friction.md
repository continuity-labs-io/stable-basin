...
- Add robust type hinting using `jaxtyping` (e.g.,
  `Float[Array, "d_state d_state"]` and `Float[Array, "d_state"]`).
...
1. Initializes a 64-dimensional `DissipativeFriction` module with a fixed PRNG
   key.
2. Asserts that the computed matrix Γ is symmetric
   (`jnp.allclose(Gamma, Gamma.T, atol=1e-5)`).
