...
def test_physics_invariant_gaussian_noise_ksm():
    ...
    for i, ksm in enumerate(ksm_scores[50:]):
        assert ksm < 0.05, (
            f"KSM at step {i + 50} is {ksm}, expected < 0.05 for pure Gaussian white noise"
        )
...
def test_physics_invariant_ksm_middle_case():
    ...
    assert 0.4 < mean_ksm < 0.7, (
        f"Expected mean KSM between 0.4 and 0.7 for mixed signal, got {mean_ksm}"
    )
