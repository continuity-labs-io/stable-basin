...
def test_mask_aware_ssm_equivalence_to_baseline():
    ...
    # They should be extremely close (the only difference is the 1e-8 shift in dt_gated in the mask
    # model)
    assert torch.allclose(out_mask, out_base, atol=1e-5), (
        "MaskAwareSSM with g=1 should match BaselineSSM"
    )
