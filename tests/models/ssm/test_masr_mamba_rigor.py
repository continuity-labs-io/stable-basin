...
def test_masr_mamba_stasis_gradient_isolation():
    ...
    testing.assert_close(
        ...
        atol=1e-8,
        rtol=1e-8,
        msg="Gradient leaked at masked time step!",
    )
...
def test_mask_aware_control_group_parity():
    ...
    model_true = MaskAwareMamba(
        input_dim=input_dim, d_model=d_model, d_state=d_state, mask_aware=True
    )
    model_false = MaskAwareMamba(
        input_dim=input_dim, d_model=d_model, d_state=d_state, mask_aware=False
    )
    ...
    assert pred_true.shape == (
        batch_size,
        seq_len,
        input_dim,
    ), f"Expected {(batch_size, seq_len, input_dim)} but got {pred_true.shape}"
...
def test_mamba_zoh_expm1_precision():
    ...
    assert not torch.allclose(B_bar_reverted.double(), B_bar_true, atol=1e-8), (
        "Test invalid! Reverted version somehow matches. The mock values might not trigger "
