...
def test_vessel_baseline_masking_invariants():
    ...
    assert torch.allclose(
        v_wall_post, torch.tensor(model.v_wall_threshold, dtype=v_new.dtype, device=v_new.device)
    ...
    assert torch.allclose(model.D_u[model.mask_wall], expected_D_u_wall), (
        "Mask constraint failed for D_u wall"
    )
