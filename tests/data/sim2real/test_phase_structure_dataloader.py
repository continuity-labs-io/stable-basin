...
def test_phase_structure_dataloader_output_format():
    loader = PhaseStructureLoader(target_components=5)
    ...
    assert "PC001" in df.columns
    assert "PC005" in df.columns
    assert list(df["Time_ms"]) == list(master_clock_ms)


def test_phase_structure_dataloader_neural_ode_smoothness():
    # The regression test that demonstrates the Neural ODE provides
    # smooth, continuously differentiable transitions.
    loader = PhaseStructureLoader(target_components=2)
