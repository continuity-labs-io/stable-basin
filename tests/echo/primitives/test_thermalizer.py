...
def create_mock_components(d_state=4, key=jax.random.PRNGKey(0)):
    ...
    ebm = PrecisionWeightedEBM(d_state=d_state, hidden_size=16, depth=2, key=key_ebm)
...
def test_thermo_flow_factor_torx_dfg():
    ...
    x_next = graph.sample(
        ...
        inputs={
            "x": x,
            "dt": dt_tensor,
            "omega_ext": jnp.zeros(d_state),
            "q_ext": jnp.zeros(d_state),
        },
        params={},
    )
...
def test_torx_thermalizer_shape_and_nans():
    ...
    thermalizer = TorxThermalizer(flow_factor=factor, n_steps=n_steps, d_state=d_state)
...
def test_torx_thermalizer_vram_xla_proof():
    ...
    n_steps = 10
    dt = 0.01
    key_comp, key_sample1, key_sample2, key_x1, key_x2 = jax.random.split(
        jax.random.PRNGKey(123), 5
    )
