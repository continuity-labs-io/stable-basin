...
def test_precision_weighted_ebm_shapes_and_constraints():
    ...
    ebm = PrecisionWeightedEBM(
        d_state=d_state, hidden_size=hidden_size, depth=depth, key=key_mod, epsilon=epsilon
    )
    ...
    assert jnp.all(eigenvalues >= epsilon - 1e-6), (
        "Precision matrix has eigenvalues smaller than epsilon."
    )
...
def test_precision_weighted_ebm_first_derivative():
    ...
    ebm = PrecisionWeightedEBM(d_state=d_state, hidden_size=hidden_size, depth=depth, key=key_mod)
...
def test_precision_weighted_ebm_second_derivative():
    ...
    assert not jnp.any(jnp.isnan(hessian_E)), (
        "Hessian contains NaNs (likely due to non-smooth activation like ReLU)."
    )
...
def test_precision_weighted_ebm_jit_and_vmap():
    ...
    assert precision_batch.shape == (
        batch_size,
        d_state,
        d_state,
    ), "Batched precision shape mismatch."
