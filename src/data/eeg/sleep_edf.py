...
class SleepEDFDataset(Dataset):
    ...
    def __init__(self, data_path: str = "data/sleep_edf/", size: int = 100, seq_len: int = 3000, n_components: int = 5):
        ...
        if os.path.exists(data_path):
            ...
            if len(edf_files) > 0:
                ...
                raw_file = os.path.join(data_path, edf_files[0])
                try:
                    # Load and get data
                    ...
                    logger.warning(f"Failed to load real Sleep-EDF data: {e}. Falling back to synthetic.")
                    self.use_synthetic = True
            else:
                logger.warning(f"No .edf files found in {data_path}. Initializing synthetic fallback for Sleep-EDF dataset.")
        else:
            logger.warning(f"Data path {data_path} not found. Initializing synthetic fallback for Sleep-EDF dataset.")
    ...
    def __getitem__(self, idx):
        if self.use_synthetic:
            # Synthetic fallback: stationary AR process noise resembling PCA components
            noise = torch.randn(self.seq_len, self.n_components)
            x_raw = torch.zeros(self.seq_len, self.n_components)
            x_raw[0] = noise[0]
            alpha = 0.95
