        ...
        if os.path.exists(data_path):
            ...
            if len(vhdr_files) > 0:
                ...
                try:
                    ...
                    logger.warning(f"Failed to load real LEMON data: {e}. Falling back to synthetic.")
                    self.use_synthetic = True
            else:
                logger.warning(f"No .vhdr files found in {data_path}. Initializing synthetic fallback for LEMON dataset.")
        else:
            logger.warning(f"Data path {data_path} not found. Initializing synthetic fallback for LEMON dataset.")
