...
class HumanTelemetryLoader(Dataset):
    ...
    def __init__(self, size: int = 100, seq_len: int = 30000):
        ...
        self.steps = {"hrv": 1, "actigraphy": 5, "temp": 250, "cgm": 15000}
    ...
    def _generate_base_signals(self):
        ...
        base_hrv = (
            800.0 + 50.0 * torch.sin(2 * math.pi * 0.25 * t) + 10.0 * torch.randn(self.seq_len)
        )
        ...
        base_temp = 37.0 + 0.5 * torch.sin(
            2 * math.pi * (1.0 / 86400.0) * t + torch.rand(1).item() * 2 * math.pi
        )
        base_temp = base_temp.unsqueeze(1)

        # CGM: Mean 100 mg/dL + slow post-prandial waves (e.g. 2 hour period)
        base_cgm = 100.0 + 30.0 * torch.sin(
            2 * math.pi * (1.0 / 7200.0) * t + torch.rand(1).item() * 2 * math.pi
        )
