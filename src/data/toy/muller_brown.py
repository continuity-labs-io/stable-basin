...
def generate_trajectory(
    key: PRNGKeyArray, state_init: jax.Array, n_steps: int, dt: float, kT: float
) -> jax.Array:
    """
...
class MullerBrownDataset(Dataset):
    ...
    def __init__(
        self, size: int, seq_len: int, dt: float = 0.001, kT: float = 15.0, seed: int = 42
    ):
        self.size = size
        ...
        centers = jnp.array([[-0.5, 1.5], [0.0, 0.5], [0.5, 0.0]])
    ...
    def __getitem__(self, idx):
        x = self.data[idx]
        mask = self.mask[idx]
        return {"x_raw": x, "mask": mask, "y_true": x.clone()}
