...
class FinalSparkDataset(Dataset):
    ...
    def __init__(
        self,
        export_dir: str = "data/ephys/finalspark/fs437_export",
        seq_len: int = 1024,
        max_events: int = None,
    ):
        ...
        if not os.path.exists(self.index_path):
            raise FileNotFoundError(
                f"Segment index {self.index_path} is required for efficient raw queries."
            )
    ...
    def __getitem__(self, idx):
        ...
        seg_mask = (
            (self.segment_index["electrode"] == electrode)
