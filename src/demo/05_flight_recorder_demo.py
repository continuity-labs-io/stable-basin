...
def main():
    ...
    for t in range(1, total_steps):
        ...
        if t > 5:
            ...
            cfc_enslavement = spectral_metrics.calculate_cfc_pac(
                z_pert_seq[:, 0], z_pert_seq[:, 1]
            ).item()
            ...
            exhaust.log_spectral_decoherence(
                plv_coherence=plv_coherence, cfc_enslavement=cfc_enslavement
            )
            ...
            vram = (
                ...
                else 1024.0 + np.sin(t * 0.1) * 50.0
            )
            perfusion = 40.0 + np.random.randn() * 2.0
            exhaust.log_infrastructure(vram_mb=vram, perfusion_rate=perfusion)

    logger.info(
        f"[+] Demo 05 Complete. You can now open {rrd_path} in the Rerun viewer by running: "
        f"rerun {rrd_path}"
    )
