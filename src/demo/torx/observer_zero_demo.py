...
def main():
    ...
    observer_factor = MacroObserverFactor(
        key=mlp_key, in_dim=in_dim, out_dim=out_dim, seq_len=seq_len
    )
    ...
    def loss_fn(model_graph, x_batch, y_batch, sample_key):
        def single_sample(x, k):
            ...
            _, aux_tup = model_graph.sample(
                k, inputs={"env_micro": x, "init_macro": init_macro}, params={}, return_aux=True
            )
            return aux_tup[0]
    ...
    X, Y = generate_data(
        data_key, batch_size=batch_size, seq_len=seq_len, in_dim=in_dim, out_dim=out_dim
    )
    ...
    init_macro = jnp.zeros(out_dim + 1, dtype=jnp.float32)
    _, aux_tup = graph.sample(
        eval_key, inputs={"env_micro": test_x, "init_macro": init_macro}, params={}, return_aux=True
    )
    pred_y = aux_tup[0]
