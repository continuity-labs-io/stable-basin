...
class HierarchicalEnslavementFactor(AbstractReferenceFactor):
    ...
    def sample(self, key, inputs, params, info=None, site_info=None, return_aux=False):
        ...
        # Note: eqx.filter_value_and_grad can be used, but jax.grad is fine since we just need wrt
        # micro.
        # We must use tree_method or just pass params if we wanted W_down gradients, but we are not
        # training!
        ...
        diffusion_micro = (
            jnp.sqrt(dt) * self.noise_scale_micro * jax.random.normal(key_micro, micro.shape)
        )
...
def main():
    ...
    chain = ChainFactor(
        base=enslavement_factor, n_steps=n_steps, feedback_porting_fn="state", weight_tied=True
    )
    ...
    def fast_sample(precision_val):
        ...
        return run_simulation(
            graph, jax.random.key(101), precision_val, n_steps, macro_dim, micro_dim
        )
    ...
    logger.info(
        f"    -> The slow macro-state prior is mathematically pacing the amplitude of the "
        f"fast micro-states."
    )
    ...
    ax3.plot(
        ...
        "w--",
        linewidth=2,
        label="Macro Prior Projection (Sample)",
    )
    ax3.set_title(
        f"Regime B: Thermodynamic Enslavement (Precision = 10) | PAC (Macro->Micro): {pac_B:.4f}"
    )
    ...
    logger.info(
        "[SUCCESS] Dashboard rendered and saved to output/demo/torx_hierarchical_enslavement.png"
    )
    logger.info("=" * 60 + "\n")
