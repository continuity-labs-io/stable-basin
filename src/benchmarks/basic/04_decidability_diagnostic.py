...
Reference:
Hahne, 2026. "Beyond Return to Baseline: Reachability, Observability, and the Measurement of
Physiological Margin"

This benchmark differentiates between two distinct failure modes in hierarchical Active Inference
systems:
1. Policy Observability Failure (Sensory Blindness): The system loses access to sensory prediction
errors,
   causing it to drift, but its internal structural integrity remains intact.
2. Reachability Collapse (Hardware/Structural Failure): The system's internal mechanical bonds
(friction)
   are destroyed, permanently eliminating its physical attractor basins.

Mechanism:
- Phase 1 (Endogenous Failure): Both patients are subjected to a constant environmental drift
(`omega_seq`)
...
Dynamics & Diagnosis:
- Patient A (Blindness) has broken sensory matrices (`D_s=0`) but an intact internal mechanical
structure
(strong friction `Gamma`). When the controller pulls the blanket into homeostasis, Patient A's
  healthy
internal mechanical bonds physically drag the core internal states back to safety. Diagnosis:
  Rescuable.
- Patient B (Structural Collapse) has destroyed internal friction matrices (`Gamma -> 0`). When the
controller pulls the blanket into homeostasis, the internal states physically detach. Without
  friction
to bind them to the blanket, they continue to drift irreversibly. Diagnosis: Irreversible
  Collapse.
"""
...
def run_simulation():
    ...
    def apply_patient_mods(tree, brake_val):
        ...
        return jax.tree_util.tree_map(
            ...
            is_leaf=lambda x: isinstance(
                x, (DissipativeFriction, SolenoidalFlow, PrecisionWeightedEBM)
            ),
        )
    ...
    traj_a_p2 = patient_a.forced_unroll(
        k3, x_drifted_a, dt, seq=None, omega_seq=omega_seq, q_gain=10.0, q_mask=q_mask
    )
    traj_b_p2 = patient_b.forced_unroll(
        k3, x_drifted_b, dt, seq=None, omega_seq=omega_seq, q_gain=10.0, q_mask=q_mask
    )
    ...
    if dist_a < rescue_threshold:
        diag_a = (
            "[DIAGNOSIS: POLICY_OBSERVABILITY_FAILURE] Hardware restored homeostasis. Physical "
            "Waddington basin is intact."
        )
        print(diag_a)
    else:
        diag_a = (
            "[DIAGNOSIS: REACHABILITY_COLLAPSE] Hardware failed. The physical attractor basin is "
...
def main():
    ...
    plt.suptitle(
        "Decidability Diagnostic\nRef: Hahne (2026) 'Beyond Return to Baseline: Reachability, "
