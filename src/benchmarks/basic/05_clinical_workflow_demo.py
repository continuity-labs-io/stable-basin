...
def calc_micro_drift(
    graph: PredictiveCodingGraph, x_micro: jax.Array, x_macro: jax.Array
) -> jax.Array:
...
def main():
    ...
    alice = PredictiveCodingGraph(
        micro_observer=micro_obs, macro_observer=macro_obs, n_steps=1, key=k3
    )
    ...
    factor_degraded = eqx.tree_at(
        lambda f: f.macro_ebm.energy_head.weight, factor, factor.macro_ebm.energy_head.weight * 0.01
    )
    factor_degraded = eqx.tree_at(
        lambda f: f.macro_ebm.precision_head.weight,
        factor_degraded,
        factor_degraded.macro_ebm.precision_head.weight * 0.01,
    )
    factor_degraded = eqx.tree_at(
        lambda f: f.W_down.weight, factor_degraded, factor_degraded.W_down.weight * 0.0
    )
    factor_degraded = eqx.tree_at(
        lambda f: f.micro_dissipative.W, factor_degraded, factor_degraded.micro_dissipative.W * 0.01
    )
    factor_degraded = eqx.tree_at(
        lambda f: f.macro_dissipative.W, factor_degraded, factor_degraded.macro_dissipative.W * 0.01
    )

    alice_degraded = eqx.tree_at(
        lambda g: g.thermalizer.graph.sites[0].factor.base, alice, factor_degraded
    )
    ...
    logging.info(
        f"STEP 2: Measuring Waddington Geometry... Trace = {hessian_trace:.4f}. Attractor "
        f"basin is flattened."
    )
    ...
    logging.info(f"Micro: {micro_surp:.4f} | Macro: {macro_surp:.4f} | Discord: {discordance:.4f}")
    if discordance < 0.1:
        logging.info(
            "DIAGNOSIS: Silent Drift Detected! Macro-level is blind to physical micro-damage."
        )
    else:
    ...
    alice_optimal = jax.tree_util.tree_map(
        lambda x: (
            eqx.tree_at(lambda p: p.energy_head.weight, x, x.energy_head.weight * 100.0)
    ...
    logging.info(
        "Twin B generated. Friction (Γ) and Precision (Π) have been restored in silico without a "
        "population database."
    )
    ...
    logging.info(
        "This is the exact exogenous energy the hardware must inject to force the physical tissue "
        "back into its youthful limit cycle."
    )
    ...
    ax2.bar(
        ["Twin A (Degraded)", "Twin B (Optimal)"],
        [hessian_trace, hessian_trace_B],
        color=["orange", "green"],
    )
