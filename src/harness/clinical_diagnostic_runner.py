...
def evaluate_model(trial_config):
    ...
    if not use_synthetic:
        ...
        if dataset_type == "aollsm":
            ...
            dataset = AOLLSMDataset(
                data_dir=data_dir, num_frames=seq_len, compressor=compressor, device=device
            )
    ...
    model = SensorFusionPredictor(
        ssm_type=model_type, modality_dims=[input_dim], d_model=d_model, out_dim=input_dim
    ).to(device)
    ...
    with open(csv_path, mode="w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["model_type", "baseline_ksm_variance", "crash_frame", "inference_latency_ms_per_frame"]
        )
    ...
    try:
        relevance_tensor = AttributionEngine.get_instance().compute_attribution(
            model, telemetry, target_time_step=crash_frame
        )
    ...
    logger.info(
        f"SUCCESS: {model_type} evaluation complete! Crash frame detected at: {crash_frame}"
    )
    wandb.finish()
