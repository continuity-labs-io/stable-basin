...
def evaluate_model(trial_config):
    ...
    logger.info(
        f"--- Running {task_name} | Model: {model_type} | Density: {density} | Seed: {seed} ---"
    )
...
def save_benchmark_plot(df_results, config, task_name, png_name):
    ...
    labels = {
        "zero_padded_ssm": "Zero-Padded SSM",
        "forward_fill_ssm": "Forward-Fill SSM",
    ...
    ax2.text(
        train_seq_len + 10, ax2.get_ylim()[1] * 0.9, "Training Horizon", color="grey", fontsize=10
    )
...
def save_density_plot(df_results, config, png_name):
    ...
    colors = {
        "zero_padded_ssm": "red",
        "forward_fill_ssm": "magenta",
        "mask_concat_ssm": "yellow",
        "gru_d": "cyan",
        "ode_rnn": "orange",
        "masr_ssm": "lime",
        "masr_mamba": "green",
    ...
    ax.invert_xaxis()
    ax.set_title(
        "OOD Generalization vs. Sensor Density (5-Seed Variance)", color="white", fontweight="bold"
    )
