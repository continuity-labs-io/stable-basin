    ...
    def __init__(
        ...
        if ssm_type == "mask_concat_ssm":
            ...
            self.fusion = OrthogonalModalityEncoder(
                d_in=d_sensor_total + num_modalities, modality_dims=modality_dims, d_model=d_model
            )
        else:
            self.fusion = OrthogonalModalityEncoder(
                d_in=d_sensor_total, modality_dims=modality_dims, d_model=d_model
            )
        ...
        if self.ssm_type == "forward_fill_ssm":
            ...
            for t in range(L):
                ...
                current_delta = torch.where(
                    m_t == 1.0, torch.zeros_like(current_delta), current_delta
                )
