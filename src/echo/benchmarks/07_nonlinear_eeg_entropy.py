...
def main():
    ...
    if acc_real > acc_surrogate + 0.05:
        logger.info("Decision Rule: Arrow-of-Time classifier successfully detected non-Gaussian irreversibility in real data that vanished in surrogates.")
    else:
    ...
    logger.info("Decision Rule: The nonlinear EBM (Estimator E) has been successfully fit to the stationary distribution, providing a non-Gaussian model of the dynamics. The linear MOU (Estimator B) provides a purely Gaussian baseline EP rate.")
