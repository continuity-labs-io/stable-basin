    ...
    def step(self, model: eqx.Module, batch: dict, key: jax.random.PRNGKey, dt: float):
        ...
        def loss_fn(trainable_model, static_model, s_true_batch, x_init_batch, step_key):
            ...
            def single_example_loss(s_true_seq, x_init, ex_key):
                ...
                # Assumes model has a 'hull' defining the Markov Blanket (e.g. MarkovBlanketObserver)
                ...
                # Compute MSE strictly between the predicted sensory slice and actual next data frame
        ...
        # Compute gradients
        loss_val, grads = eqx.filter_value_and_grad(loss_fn)(trainable, static, batch['s_true'], batch['x_init'], key)
