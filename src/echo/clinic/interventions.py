    ...
    def anneal_twin(
        ...
        def _map_fn(x):
            ...
            if isinstance(x, PrecisionWeightedEBM):
                return eqx.tree_at(
                    lambda p: p.precision_head.weight, x, x.precision_head.weight * pi_boost
                )
            return x
...
class DigitalTwinInterrogator(eqx.Module):
    ...
    def ping_and_measure(
        ...
        """
                ...
                Args:
                    graph: A PredictiveCodingGraph containing the hierarchy's physics.
                    x_micro: The full state vector of the micro-level observer.
                    x_macro: The full state vector of the macro-level observer.
        q_ext_pulse: An exogenous perturbation vector representing a bioelectric shock applied
                    to the micro level.

                Returns:
                    dict: A dictionary containing:
        - "micro_surprisal": The L2 norm of the free energy gradient with respect to the
                        shocked micro state.
        - "macro_surprisal": The L2 norm of the free energy gradient with respect to the
                        macro state.
        - "discordance": The ratio of macro surprisal to micro surprisal (measuring Sensory
                        Blindness).
        """
