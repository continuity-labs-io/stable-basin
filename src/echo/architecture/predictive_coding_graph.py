...
class PredictiveCodingGraph(eqx.Module):
    ...
    def __init__(
        self,
        micro_observer: MarkovBlanketObserver,
        macro_observer: MarkovBlanketObserver,
        n_steps: int,
        key: PRNGKeyArray,
    ):
        # We need a hull that represents the concatenated state so apply_sensory_degradation works
        # safely.
        ...
        self.hull = MarkovHull(
            ...
            d_external=micro_observer.hull.d_external
            + macro_observer.hull.d_state
            - macro_observer.hull.d_internal
            - micro_observer.hull.d_sensory
            - micro_observer.hull.d_active,
        )
        ...
        self.thermalizer = TorxThermalizer(flow_factor=factor, n_steps=n_steps, d_state=d_state)
        self.forced_thermalizer = ForcedTorxThermalizer(
            flow_factor=factor, d_state=d_state, injection_start_idx=micro_observer.hull.d_internal
        )
