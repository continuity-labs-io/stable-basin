...
class HierarchicalThermoFlowFactor(torx.factor.AbstractReferenceFactor):
    ...
    def sample(self, key, inputs, params, info=None, site_info=None, return_aux=False):
        x = inputs["x"]
        dt = inputs["dt"]
        omega_ext = inputs.get(
            "omega_ext", jnp.zeros(self.d_micro + self.d_macro, dtype=jnp.float32)
        )
        ...
        if "S_micro" in params:
            ...
            logging.warning(
                "HierarchicalThermoFlowFactor fallback to slow jnp.linalg.eigh for S_micro"
            )
        ...
        if "S_macro" in params:
            ...
            logging.warning(
                "HierarchicalThermoFlowFactor fallback to slow jnp.linalg.eigh for S_macro"
            )
