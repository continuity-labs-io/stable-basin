...
class ActiveInferenceAgent(nn.Module):
    ...
    def __init__(
        ...
        self.u[..., s_min:s_max, s_min:s_max] = (
            torch.rand(1, 1, s_max - s_min, s_max - s_min) * 2 - 1
        )
        self.v[..., s_min:s_max, s_min:s_max] = (
            torch.rand(1, 1, s_max - s_min, s_max - s_min) * 2 - 1
        )
        ...
        kernel_lap = torch.tensor(
            [[0.05, 0.20, 0.05], [0.20, -1.00, 0.20], [0.05, 0.20, 0.05]], dtype=torch.float32
        )
        ...
        sobel_x = torch.tensor(
            [[-0.125, 0.0, 0.125], [-0.250, 0.0, 0.250], [-0.125, 0.0, 0.125]], dtype=torch.float32
        )
        self.register_buffer("grad_x_kernel", rearrange(sobel_x, "h w -> 1 1 h w"))

        sobel_y = torch.tensor(
            [[-0.125, -0.250, -0.125], [0.000, 0.000, 0.000], [0.125, 0.250, 0.125]],
            dtype=torch.float32,
        )
    ...
    def _apply_conv(
        self, state: Float[torch.Tensor, "batch channels height width"], kernel: torch.Tensor
    ...
    def compute_gradient(
        self, state: Float[torch.Tensor, "batch channels height width"]
    ) -> tuple[
        Float[torch.Tensor, "batch channels height width"],
        Float[torch.Tensor, "batch channels height width"],
    ]:
    ...
    def compute_divergence(
        self,
        fx: Float[torch.Tensor, "batch channels height width"],
        fy: Float[torch.Tensor, "batch channels height width"],
    ) -> Float[torch.Tensor, "batch channels height width"]:
