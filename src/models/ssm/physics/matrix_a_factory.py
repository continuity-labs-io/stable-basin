...
class LogSpacedAInit(nn.Module):
    """
        ...
        Maps initialization frequencies to target biological timescales.
    high-frequency electrophysiology (e.g., 20,000 Hz) to slow morphological drift (e.g., 0.0001
        Hz).
        ...
        Args:
            shape (tuple): The required shape of the parameter.
...
class RandomAInit(nn.Module):
    ...
    def __init__(self, shape: tuple[int, ...], a_scale: float = 0.5, a_shift: float = 0.1):
        ...
        self.A_log = nn.Parameter(
            torch.log(torch.clamp(torch.rand(shape), min=1e-4) * a_scale + a_shift)
        )
...
def create_a_matrix(init_type: str, shape: tuple[int, ...], **kwargs) -> nn.Module:
    ...
    if init_type == "log_spaced":
        ...
        raise ValueError(
            f"Unknown A matrix init_type: {init_type}. Must be 'random' or 'log_spaced'."
        )
