...
def mamba_masr_reference_scan(
    ...
    B_bar = (
        torch.expm1(dt_masked_exp * A) / A_safe * B.unsqueeze(2)
...
class PyTorchMambaMASR(nn.Module):
    def __init__(self, d_model, d_state=16, a_init_type: str = "random"):
        ...
        self.A_init = create_a_matrix(
            init_type=a_init_type, shape=(d_model, d_state), a_scale=0.5, a_shift=0.1
        )
    ...
    def forward(
        ...
        mask: Float[Tensor, "batch seq_len d_model"],
...
class MaskAwareMamba(nn.Module):
    ...
    def forward(
        ...
        mask: Float[Tensor, "batch seq_len input_dim"] | None = None,
