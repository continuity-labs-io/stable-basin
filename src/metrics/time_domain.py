...
class ThermodynamicMetrics:
    ...
    def calculate_csd(self, z_sequence, window_size=settings.CSD_WINDOW_SIZE):
        ...
        for t in range(window_size, time_steps + 1):
            ...
            # Mean-center the sliding window to compute true variance/correlation
            z_win_active = z_win_active - z_win_active.mean(dim=0, keepdim=True)
    ...
    def calculate_ksm(
        self,
        z_sequence,
        window_size=settings.KSM_WINDOW_SIZE,
        debug_crash_frame=None,
        rank_method="default",
    ):
        """
        The code snippet implements Dynamic Mode Decomposition using a truncated Singular Value
                Decomposition.
        ...
        The decision to utilize Dynamic Mode Decomposition over pseudo-arc length continuation stems
                from
                the specific architectural constraints of the platform.
                - Compute Latency: Pseudo-arc length continuation is an iterative
                root-finding algorithm.
                It is computationally expensive and risks introducing variable execution times.
        Truncated Singular Value Decomposition over a small sliding window is deterministic and
                executes
        with high efficiency on edge GPUs, ensuring the metric keeps pace with the biological
                timescales
                of milliseconds to minutes.
                - Model Independence: Pseudo-arc length continuation requires an explicit,
                differentiable
                non-linear vector field to compute the Jacobian. Because the tissue
                trajectory is modeled
                inside the continuous latent space of the state-space engine, defining
                the exact non-linear
                continuous field is complex.
        Dynamic Mode Decomposition is entirely data-driven, extracting the kinetic modes directly
                from
                the streaming embeddings without requiring the underlying equations.
                - Architectural Simplicity: The current approach provides a fast, elegant
                solution that
        ...
        for t in range(window_size, time_steps):
                ...
                with warnings.catch_warnings():
                    ...
                    try:
                        if rank_method == "dynamic":
                            # Compute SVD to find rank dynamically using Gavish-Donoho + Log-
                            # Spectral Gap
                        ...
                        if debug_crash_frame is not None:
                            ...
                            if t == debug_crash_frame - 1:
                                logger.debug(
                                    f"PyDMD Audit [BEFORE crash, t={t}]: "
                                    f"eigs={eigenvalues}, max_eig={max_eig}"
                                )
                            elif t == debug_crash_frame + 1:
                                logger.debug(
                                    f"PyDMD Audit [AFTER crash, t={t}]: "
                                    f"eigs={eigenvalues}, max_eig={max_eig}"
                                )
            ...
            logger.debug(
                f"[PyDMD] Frame {t} | temporal_std={temporal_std:.6f} | "
                f"max_eig={max_eig:.4f} | KSM={ksm:.4f}"
            )
    ...
    def calculate_lle(self, z_sequence, window_size=settings.LLE_WINDOW_SIZE, dt=1.0):
        ...
        for t in range(window_size, time_steps):
            ...
            logger.debug(
                f"[PyDMD] Frame {t} | temporal_std={temporal_std:.6f} | "
                f"max_eig={max_eig:.4f} | LLE={lle:.4f}"
            )
    ...
    def extract_fedichev_macrostates(
        ...
        window_size: int = 4,
        cpg_tensor: torch.Tensor = None,
    ) -> dict:
        """
        Extracts the three macroscopic variables defining the Fedichev-Gruber minimal model of
                aging:
        z0 (fast dynamic stress response), Z (slow cumulative entropic damage), and epsilon_0
    ...
    def calculate_unified_diagnostics(
        self, z_seq, raw_telemetry, macro_channel_idx, micro_channel_idx, sampling_rate
    ):
        """
        ...
        return {
            "time_domain": {"ksm": ksm_scores, "csd": csd_scores},
            "frequency_domain": {"psd": (freqs, psd), "pac": pac},
        }
