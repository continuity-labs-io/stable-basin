...
def get_patient_journal(bio_age):
    if bio_age > 70:
        return (
            Colors.RED
            + "Patient Journal: 'The procedure nearly killed me. I was bedridden for months. I feel "
            "so frail.'" + Colors.ENDC
        )
    elif bio_age > 50:
        return (
            Colors.YELLOW
            + "Patient Journal: 'It was a slog, but I'm recovering faster than before. I can feel "
            "my joints loosening.'" + Colors.ENDC
        )
    elif bio_age > 35:
        return (
            Colors.CYAN
            + "Patient Journal: 'I bounced back in just a few weeks! I'm running again. This is "
            "incredible.'" + Colors.ENDC
        )
    else:
        return (
            Colors.GREEN
            + "Patient Journal: 'I feel invincible! The recovery was barely a weekend. I am getting "
            "my life back!'" + Colors.ENDC
        )
...
def run_interactive_simulation():
    ...
    while True:
        ...
        try:
            start_age = float(
                input(Colors.BOLD + "Enter the patient's starting age: " + Colors.ENDC)
            )
        except ValueError:
        ...
        if start_age >= 90:
            print(
                Colors.RED + Colors.BOLD + "WARNING: EXTREME FRAILTY INDEX DETECTED." + Colors.ENDC
            )
            print_slow(
                "Patient's biological structures cannot survive the physical shock of Level 3 "
                "Rejuvenation."
            )
            print_slow(
                "Recommendation: Immediate Cryopreservation until Level 4 (in-situ cellular "
                "reprogramming) is available."
            )
            ...
            while bio_age > 20:
                ...
                plt.clear_figure()
                plt.plot(
                    history_years, history_chrono, label="Chrono Age", color="blue", marker="dot"
                )
                ...
                print(
                    f"Current Biological Age:    {get_color_for_age(bio_age)}{bio_age:.1f}{Colors.ENDC}\n"
                )
                ...
                if recovery_days > 365:
                    ...
                    print_slow(
                        "Clinic policy prohibits treatments requiring >1 year of continuous bed "
                        "rest."
                    )
                ...
                print_slow(
                    f"  └─ Recovery Time:  {color}{recovery_days} days bedridden{Colors.ENDC}",
                    delay=0.01,
                )
                print_slow(
                    f"  └─ Post-op BioAge: {color}{bio_age:.1f} years{Colors.ENDC}", delay=0.01
                )
                ...
                if bio_age <= 20:
                    ...
                    print_slow(
                        f"Total Time Bedridden: {total_recovery_days} days ({total_recovery_days / 365.25:.1f} years)"
                    )
                ...
                print(
                    Colors.CYAN
                    + f"Doctor's orders: 'Come back in {int(years_between_treatments)} years for your next round.'"
                    + Colors.ENDC
                )
                print("Time passes...\n")
                time.sleep(1)
        ...
        print("\n" + "=" * 60)
        play_again = (
            input(Colors.BOLD + "Would you like to simulate another patient? [Y/n]: " + Colors.ENDC)
            .strip()
            .lower()
        )
        if play_again == "n":
