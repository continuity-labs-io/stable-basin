...
def main():
    ...
    loader_50 = EpigeneticEntropyLoader(
        biological_age=50, size=1, seq_len=1, n_cells=100, n_cpgs=100
    )
    loader_45 = EpigeneticEntropyLoader(
        biological_age=45, size=1, seq_len=1, n_cells=100, n_cpgs=100
    )
    ...
    ax.axvline(abort_min, color="red", linestyle="--", linewidth=2)
    ax.set_title(
        "Critical Slowing Down (D_0 Biological Noise)", color="magenta", fontsize=14, pad=10
    )
    ...
    ax.set_title(
        "Configurational Entropy (Z - Biological Age)", color="yellow", fontsize=14, pad=10
    )
