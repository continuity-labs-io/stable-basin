...
def pytorch_fused_scan(
    ...
    for i in range(max_events):
        ...
        h_active = h.gather(
            1, safe_sensor_ids.unsqueeze(1).unsqueeze(2).expand(-1, -1, d_state)
        ).squeeze(1)
        h_new = A_bar * h_active + B_bar

        # 4. Vectorized Scatter (Safely mapping back into full tensor)
        h_scatter = h.scatter(
            1, safe_sensor_ids.unsqueeze(1).unsqueeze(2).expand(-1, -1, d_state), h_new.unsqueeze(1)
        )
...
class AsyncMaskAwareSSM(nn.Module):
    ...
    def forward(self, events: torch.Tensor, event_mask: torch.Tensor):
        ...
        if HAS_TRITON and events.device.type == "cuda":
            ...
            return pytorch_fused_scan(
                events, event_mask, A_log, self.B_proj, self.dim, self.d_state
            )
